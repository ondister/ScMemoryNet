...
namespace ScMachineWrapperTest.SafeElements
{
    ...
    public class ScLinkContentTests
    {
        ...
        {
            var imageStreamSource = new FileStream("images/testimage.png", FileMode.Open, FileAccess.Read, FileShare.Read);
          
            var bytes = new byte[imageStreamSource.Length];
           imageStreamSource.Read(bytes,0,bytes.Length);

            ScBitmap content = new ScBitmap(bytes);
        ...
        [TestMethod]
        public void CreateScTimeIntervalLinkTest()
        {
            var content = new ScTimeInterval(new TimeSpan(3,2,1));
            var linkWithContent = context.CreateLink(content);
            var findedLinks = context.FindLinks(content);
            Assert.IsTrue(findedLinks.Count == 1);
            var findedLinkContent = findedLinks[0].LinkContent;
            Assert.AreEqual(linkWithContent.LinkContent, findedLinkContent);
            Assert.IsTrue(content == findedLinkContent);
            Assert.AreEqual(typeof(ScTimeInterval), findedLinkContent.GetType());
        ...
        [TestMethod]
        public void CreateScDateLinkTest()
        {
            var content = new ScDate(DateTime.Today);
            var linkWithContent = context.CreateLink(content);
            var findedLinks = context.FindLinks(content);
            Assert.IsTrue(findedLinks.Count == 1);
            var findedLinkContent = findedLinks[0].LinkContent;
            Assert.AreEqual(linkWithContent.LinkContent, findedLinkContent);
            Assert.IsTrue(content == findedLinkContent);
            Assert.AreEqual(typeof(ScDate), findedLinkContent.GetType());
        ...
        [TestMethod]
        public void CreateScDateTimeLinkTest()
        {
            var content = new ScDateTime(DateTime.Today);
            var linkWithContent = context.CreateLink(content);
            var findedLinks = context.FindLinks(content);
            Assert.IsTrue(findedLinks.Count == 1);
            var findedLinkContent = findedLinks[0].LinkContent;
            Assert.AreEqual(linkWithContent.LinkContent, findedLinkContent);
            Assert.IsTrue(content == findedLinkContent);
            Assert.AreEqual(typeof(ScDateTime), findedLinkContent.GetType());
