...
namespace Demo
{
    public class Demo
    {
        ...
        public void Start()
        {
            ...
            classLinkForTokenize.AddOutputArc(link, ElementType.PositiveConstantPermanentAccessArc_c);


            var link1 = context.CreateLink("доктор опять в шкафу");
            classLinkForTokenize.AddOutputArc(link1, ElementType.PositiveConstantPermanentAccessArc_c);
            Console.ReadKey();

           //запрос
            var classQuerryString = context.FindNode("class_querry_string");
            var querryInstNode = context.CreateNode(ElementType.ConstantNode_c);
            ...
            //создаем запрос
            classQuerryString.AddOutputArc(querryInstNode, ElementType.PositiveConstantPermanentAccessArc_c);
        ...
        void classQuerryString_OutputArcAdded(object sender, ScEventArgs e)
        {
            ...
            { 
                ...
                var constrIterator = context.CreateIterator(nrelQuerryResponse, ElementType.PositiveConstantPermanentAccessArc_c, e.Arc);
